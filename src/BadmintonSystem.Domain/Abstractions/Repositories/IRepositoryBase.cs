...
namespace BadmintonSystem.Domain.Abstractions.Repositories;
...
public interface IRepositoryBase<TEntity, in TKey> where TEntity : class
{
    ...
    IQueryable<T> ExecuteSqlQuery<T>(FormattableString sql);

    IQueryable<T> ExecuteSqlQueryRaw<T>(string sql, params object[] parameters);
}
