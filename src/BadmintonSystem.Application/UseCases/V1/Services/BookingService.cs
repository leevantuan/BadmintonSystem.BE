...
public class BookingService(
    ...
    : IBookingService
{
    public async Task SignalRAndUpdateCacheAsync(BusCommand.SendUpdateCacheBusCommand request, CancellationToken cancellationToken)
    {
        try
        {
            foreach (Contract.Services.V1.Booking.Response.GetIdsByDate yardPrice in request.YardPriceIds)
            {
                // Real-time SignalR
                ...
                {
                    await sender.Send(
                        new Contract.Services.V1.YardPrice.Query.GetYardPricesByDateQuery(Guid.Empty, yardPrice.Date, request.Name),
                        cancellationToken);
